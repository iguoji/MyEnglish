...
从任意复习模块返回首页时，顶部「今日复习 X/目标」、四张模式卡的百分比、复习趋势曲线和复习质量日历会一起重新统计，不需要重启应用就能看到刚才的成绩。
...
- 点一张左卡再点一张右卡，配对正确会画出一条绿色曲线并划掉这一对；配对错误则两张卡红框抖动。
...
- 中途退出会冻结进度，当天再次进入可以接着玩。
- 词义连连的对错**不写入复习记录**，也不改变单词难度，因此首页该模式的百分比来自本局进度而不是复习次数。
