...
当前导出文件不包含复习历史记录。“清空数据”会连同复习记录、模块会话与每日词库一起删除。请不要把导出文件理解为整个应用的完整镜像。
...
Android 导入层通过 SQLite `PRAGMA table_info` 读取真实字段作为白名单：存在的字段按数据库类型写入，未知字段忽略。导入是整库替换，不是追加；完整备份可以恢复分组和成员关系，但不包含复习记录、候选缓存和学习会话。
