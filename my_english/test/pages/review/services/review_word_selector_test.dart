...
void main() {
  ...
  test('都没复习过时依次比难度、含义条数、含义字数、拼写、编号', () {
    final words = <Word>[
      // 释义两条、难度 0：难度最低且条数最多，排最后。
      _word(10, 'aaa', definitions: <String>['甲', '乙']),
      // 一条释义但正文更长、难度 0：难度同 10 但条数少，排在 10 前面。
      _word(11, 'bbb', definitions: <String>['很长的一条释义']),
      // 一条短释义、难度 3：难度最高，排最前。
      _word(12, 'ccc', definitions: <String>['短'], difficulty: 3),
      // 一条同样长度的释义、难度 1：难度次于 12，排在 12 后面。
      _word(13, 'ddd', definitions: <String>['短'], difficulty: 1),
    ];

    // 难度降序 → 条数升序 → 字数升序。
    // 12(3) > 13(1) > 10(0)=11(0)；10 与 11 难度相同再比含义条数，11 少排前。
    expect(
  ...
  test('两层选词：第一层优先抓难度最高的词', () {
    ...
    // 第二层排除 {3,4} 后剩 [1,2]，按第二层规则（复习时间全空→难度降序→
    // 含义全同）排成 [2, 1]。
  ...
  test('两层选词：第二层按复习时间排，难度只在复习时间相同时才生效', () {
    ...
    // 注意 d 难度(2) 比 b 难度(1) 高，但第二层复习时间打头、难度第二位，
    // 只有复习时间相同时难度才介入——这正是两层规则的意义。
    expect(
      _ids(ReviewWordSelector.selectTwoLayer(words, limit: 4)),
      <int>[1, 3, 2, 4],
    );
  ...
  test('两层选词：10 个词按 40/60 分配，第一层 4 个第二层 6 个', () {
    ...
    // 第二层排除后剩难度 1~6 的词，复习时间全空时第二层难度降序先介入，
    // 含义全同不再扰动顺序 → [6,5,4,3,2,1]。
