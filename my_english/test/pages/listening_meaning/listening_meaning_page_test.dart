...
void main() {
  ...
  testWidgets('listeningMeaning handles wrong answers and advances between words', (
    ...
    // 顶栏右上角难度徽章已下线，只需确认题号与候选区仍正常展示。
    // 播放是右侧主操作，使用蓝色背景和白色前景。
    final playButton = tester.widget<OutlinedButton>(
      find
          .descendant(
            of: find.byKey(const Key('listening-meaning-play')),
  ...
  testWidgets('completion is recorded only after tapping next', (tester) async {
    ...
    const channel = MethodChannel('test/listeningMeaning_record_timing');
    // 保存原生事务收到的完整调用参数。
    ...
    await tester.tap(find.byKey(const Key('listening-meaning-hint')));
    await tester.pump();
    await tester.tap(find.text('abliity'));
    await tester.pump();
    ...
    final lockedNextButton = tester.widget<FilledButton>(
      find.byKey(const Key('next-listening-meaning-word')),
    );
    expect(lockedNextButton.onPressed, isNull);
    // 保存期间左上角返回不响应，系统返回对应的路由策略也必须是 doNotPop。
    await tester.tap(find.byKey(const Key('close-listeningMeaning')));
    await tester.pump();
    expect(find.byType(ListeningMeaningPage), findsOneWidget);
    final listeningMeaningRoute = ModalRoute.of(
      tester.element(find.byType(ListeningMeaningPage)),
    );
  ...
  testWidgets('last completion submits before opening the done page', (
    ...
    // 为最后一题建立独立通道并记录调用次数。
    const channel = MethodChannel('test/listeningMeaning_last_word');
    final nativeCalls = <MethodCall>[];
    messenger.setMockMethodCallHandler(channel, (call) async {
      nativeCalls.add(call);
      return null;
  ...
  testWidgets('retry button resets the current word to its initial state', (
    ...
    _expectFourOptions(tester);
    expect(find.byKey(const Key('listening-meaning-hint')), findsOneWidget);
    expect(find.byKey(const Key('listening-meaning-play')), findsOneWidget);
    // 完成态的两个按钮消失。
  ...
  testWidgets('listeningMeaning hint reveals leading letters progressively', (
    tester,
  ) async {
    await tester.pumpWidget(
      MaterialApp(
  ...
  testWidgets('question overlay replays without intercepting bottom controls', (
    ...
    // 显式播放按钮仍只触发一次相同播放事件。
    await tester.tap(find.byKey(const Key('listening-meaning-play')));
    await tester.pump();
    expect(player.requested, <String>[
      'ability',
      'ability',
  ...
  testWidgets('listeningMeaning mirrors listening header and anchors split controls', (
    ...
    for (var index = 1; index < optionRects.length; index++) {
      expect(
        ...
        closeTo(
          optionRects[index - 1].bottom + ListeningMeaningLayout.optionGap,
          0.01,
        ),
      );
    }
    ...
    final hintRect = tester.getRect(find.byKey(const Key('listening-meaning-hint')));
    // 提示命令使用灯泡图标，不再只有一段孤立文字。
    expect(
      find.descendant(
        of: find.byKey(const Key('listening-meaning-hint')),
        matching: find.byIcon(TablerIcons.bulb),
      ),
      findsOneWidget,
    );
    // 右侧播放按钮作为最后一项，底边对齐第四个候选词。
    final playRect = tester.getRect(find.byKey(const Key('listening-meaning-play')));
    expect(
      playRect.top,
      closeTo(hintRect.bottom + ListeningMeaningLayout.optionGap, 0.01),
    );
...
List<String> _visibleOptionTexts(WidgetTester tester) {
  ...
  return <String>[
    for (var index = 0; index < 4; index += 1)
      tester
          .widget<Text>(find.byKey(Key('listening-meaning-option-label-$index')))
          .data!,
  ];
}
