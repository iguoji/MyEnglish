...
class MainActivity : FlutterActivity() {
    ...
    // 独立 I/O 队列处理缓存扫描与 SAF 文件，避免占用数据库队列。
    ...
    override fun configureFlutterEngine(flutterEngine: FlutterEngine) {
            ...
            .setMethodCallHandler { call, result ->
                ...
                when (call.method) {
                    ...
                    "getWordsByIds" -> runDatabaseCall(result) {
                        wordsDatabase.getWordsByIds(readLongList(call.arguments, "getWordsByIds"))
                    }

                    // 按含义主键反查所属单词；看义选词恢复会话时用。
                    "getWordsByMeaningIds" -> runDatabaseCall(result) {
                        wordsDatabase.getWordsByMeaningIds(
                            readLongList(call.arguments, "getWordsByMeaningIds"),
                        )
                    }

                    // 新增单词；主体与含义在同一个事务里写入，返回自增主键。
                    "createWord" -> runDatabaseCall(result) {
                        val payload = call.arguments as? Map<*, *> ?: error("createWord 缺少参数")
                        wordsDatabase.createWord(payload)
                    }

                    // 更新单词；含义整体替换，任何一步失败都整体回滚。
                    "updateWord" -> runDatabaseCall(result) {
                        val payload = call.arguments as? Map<*, *> ?: error("updateWord 缺少参数")
                    ...
                    "deleteWord" -> runDatabaseCall(result) {
                        wordsDatabase.deleteWord(readLong(call.arguments, "id", "deleteWord"))
                        null
                    }
                    ...
                    "saveWordSyllables" -> runDatabaseCall(result) {
                        val payload = call.arguments as? Map<*, *>
                            ?: error("saveWordSyllables 缺少参数")
                        wordsDatabase.saveWordSyllables(
                            readLong(call.arguments, "wordId", "saveWordSyllables"),
                            payload["syllables"],
                        )
                        null
                    }

                    // 按复习规则挑单词；layer 决定用「难度优先」还是「久未复习优先」。
                    "pickWords" -> runDatabaseCall(result) {
                        val payload = call.arguments as? Map<*, *> ?: error("pickWords 缺少参数")
                        wordsDatabase.pickWords(
                            (payload["limit"] as? Number)?.toInt() ?: error("pickWords 缺少 limit"),
                            readLongList(payload["exclude"] ?: emptyList<Any?>(), "pickWords.exclude"),
                            (payload["layer"] as? Number)?.toInt() ?: WordsDatabase.LAYER_STALE,
                        )
                    }
                    ...
                    // 导入完整备份：先清空，再整库替换，主键原样保留。
                    "importData" -> runDatabaseCall(result) {
                        val payload = call.arguments as? Map<*, *> ?: error("importData 缺少参数")
        ...
        // 注册全局设置通道；设置现在也住在同一个数据库的 settings 表里，
        // 因此统一走数据库队列，不再有第二份 SharedPreferences。
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, settingsChannelName)
            .setMethodCallHandler { call, result ->
                when (call.method) {
                    // 一次读出全部设置：{ key: {value, type} }。
                    "getSettings" -> runDatabaseCall(result) {
                        wordsDatabase.getSettings()
                    }

                    // 写入一个设置项；有就改，没有就建。
                    "setSetting" -> runDatabaseCall(result) {
                        val payload = call.arguments as? Map<*, *> ?: error("setSetting 缺少参数")
                        val key = payload["key"]?.toString()?.trim()?.takeIf { it.isNotEmpty() }
                            ?: error("setSetting 缺少有效 key")
                        val value = payload["value"]?.toString() ?: error("setSetting 缺少 value")
                    ...
                    // 清空全部设置，恢复到首次安装的默认值。
                    "clearSettings" -> runDatabaseCall(result) {
                        wordsDatabase.clearSettings()
                        null
                    }
     ...
     *
     * @param arguments Dart 传来的原始参数，应为 Map。
     * @param field 要读取的字段名。
     * @param method 方法名，仅用于拼出可读的错误信息。
     * @return 转换后的 Long 值。
     */
    private fun readLong(arguments: Any?, field: String, method: String): Long {
        val payload = arguments as? Map<*, *> ?: error("$method 缺少参数")
        return (payload[field] as? Number)?.toLong() ?: error("$method 缺少有效 $field")
    }
